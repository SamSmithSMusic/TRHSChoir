    ...
    for (const key in groupedData) {
        ...
        choirYearData.forEach(performance => {
            ...
            while (vidcheck == 0) {
                ...
                aWrap.setAttribute('href',`../Performances/index.html?key=${encodeURIComponent(keySend)}`);
...
const sort = (performances) => {
    ...
    if (year != "all" && choir != "all") {
        displayRepertoire(performances.filter(performance => performance.Choir.includes(choir) && performance.Year.includes(year)));
    }
    else if (year != 'all') {
        displayRepertoire(performances.filter(performances => performances.Year.includes(year)));
    }
    else if (choir != 'all') {
        displayRepertoire(performances.filter(performances => performances.Choir.includes(choir)));
    }
    else {
        displayRepertoire(performances);
    }
...
document.querySelector("#sortYear").addEventListener("change", () => {sort(performances)});
document.querySelector("#sortChoir").addEventListener("change", () => {sort(performances)});